...
class Field
{
    ...
    Field()
    {
        ...
        this.initField();
        apple = new Coord(16, 2);
        //apple = new Coord(12, 7);
        field[apple.getY()][apple.getX()].changeType(2);
    }
    ...
    private void initField()
    {
        ...
        for (int i = 0; i < field.length; i++)
        {
            ...
            {
                field[i][j] = new Unit(new Coord(x, y), 0);
                x += 25;
            }
            x = 0;
            y += 25;
    ...
    Coord getMove(Coord position, int direction, Snake snaky)
    {
        Coord target;
        switch (direction)
        {
        ...
        if (this.field[target.getY()][target.getX()].getType() == 2) respawnApple(snaky);
        return target;
    }

    void respawnApple(Snake snaky)
    {
        int x = apple.getX();
        int y = apple.getY();
        ...
        do
        {
            temp.setX(1+random.nextInt(field[0].length-2));
            temp.setY(1+random.nextInt(field.length-2));
        }
        while (field[temp.getY()][temp.getX()].getType() == 1);
        apple.setX(temp.getX());
        apple.setY(temp.getY());
        field[apple.getY()][apple.getX()].changeType(2);
        snaky.grow();
    }

    Unit[][] getField() { return field; }
    Color[] getFieldColors() { return fieldColors; }
    Coord getBoundReal() { return boundReal; }
    Coord getBoundIndex() { return boundIndex; }
